	      ...
	      schedule_endtime:{
	    	  verbose: false,
	            validators: {
	                ...
	                callback: {
                        message: 'Schedule end time should be greater than start time',
                       
                        callback: function(value, validator, $field) {
                        	
                            var startTime = validator.getFieldElements('schedule_starttime').val();
                            var endTime = validator.getFieldElements('schedule_endtime').val();
                            
                           /* if(startTime==null || startTime=='')
                            	{
                            	    $('#errormsg').html('Plase enter schedule start time first');
                            	    $('#schedule_endtime').attr('disabled', true);
                            	    $('#schedule_endtime').val()=='';
                            ...
                            var dtStart_time = new Date("1/1/2011 " + startTime);
                   		    var dtEnd_time = new Date("1/1/2011 " + endTime);
                   		  ...
                   		  if (difference_in_milliseconds > 1)
                   		 {
                   			 ...
                   			 validator.updateStatus('schedule_starttime', validator.STATUS_VALID, 'callback');
                             return true;
                   		 }
		 	...
		 	$.ajax({    //create an ajax request to display       
		       ...
		       //   $("#event_id").val(""+response[0].event_id).trigger("change");		      
		         ...
		         //  $("#about_us").val(""+(response[0].about_us?response[0].about_us:"")).trigger("change");
		          ...
		          $("#schedule_date").val(""+(moment(response[0].schedule_date).format("DD-MM-YYYY")?moment(response[0].schedule_date).format("DD-MM-YYYY"):"")).trigger("change");
...
function enableStartEndDate(s,e){
	...
	$('#'+s).datetimepicker({format: 'DD-MM-YYYY',  minDate: truncateDate(new Date()),useCurrent: false // <-- THIS WORKS, useCurrent: false //Important! See issue #1075
		});
	 ...
	 $.ajax({    //create an ajax request to display.php
	        ...
	        success: function(response){                    
	            ...
	            if(response[y].schedule_date!=null){
	            	...
	            	//alert(schedule_date1);
	            	//alert("sssssssss"+schedule_date);
	            // scdule_date=moment(schedule_date).format("DD/MM/YYYY");
	            	//var date1 = moment(schedule_date, 'DD-MM-YYYY')
	            	//alert(moment(schedule_date).format("DD/MM/YYYY"));
              ...
              '  <div  id="row_event_aboutus_'+response[y].id+'"   > '+  (response[y].description?response[y].description:'') +'</div></div></div></td>'+
                ...
                '<td><div class="form-group"><div class="input-group" id="schedule_dateDIV_'+response[y].id+'" >'+
                ' <div class="col-sm-12">'+
              '  <div id="row_schedule_date_'+response[y].id+'" > '+  (moment(response[y].schedule_date).format("DD-MM-YYYY")?moment(response[y].schedule_date).format("DD-MM-YYYY"):'') +'</div></div> '+
              '</div> </div></td>'+
